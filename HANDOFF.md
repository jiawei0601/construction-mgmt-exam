...
- ▶️2026-08-04 十版（職安三職類術科詳解，進行中）：
  ...
  - ✅補圖完成（commit 4735e9b）：38 張原卷圖表裁入詳解（osh 4/safety 19/hygiene 14＋1），
    含潛水減壓表、GHS 危害圖式對照表、電路圖、失誤樹、現場照片等；圖檔↔引用零斷鏈。
    **補圖意外救回 4 題實質錯誤**：safety 110-3 共同接地漏算變壓器側 3Ω（故障電流應 40A、
    外殼 80V）、safety 114-3 未辨識圖一接地導體 vs 圖二設備接地（27.5mA vs ≈0A）、
    hygiene 108-1 照度 A 區位置誤植（620 Lux、整體 610 Lux 雙法驗算）、
    hygiene 113-1 流程圖 C/D 對調（正解 4A2B1C3D）。
  - ✅頁面：osh 150／safety 165／hygiene 165 大題摺疊，覆蓋率提示改動態計算；
    清除 25 卷被誤渲染成大題的 agent「## 自檢結果」段落。
