...
- ▶️2026-08-04 七版進行中（使用者核准）：新增職安乙級子系統（共用架構、一站兩職類）。
  範圍=先建學術科題庫、詳解候補。架構決策：根路徑維持營造甲級不動＋頂部職類切換列，
  職安乙級掛 osh/（資料 raw-osh/、osh/data/questions.js、localStorage 前綴 oshexam_）；
  共同科目 c- 400 題與其既有解析直接共用。流程：下載(進行中)→解析→站台整合→發佈。
